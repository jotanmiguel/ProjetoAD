...
import random
import socket as s
from ticker_pool import lock_pool
...
class resource:
    ...
    def status(self, client_id:int):
        ...
        if client_id not in self.subList.keys():
            return [31,"UNSUBSCRIBED"]
        elif client_id in self.subList.keys():
            return [31,"SUBSCRIBED"]
    ...
    def __repr__(self):
        ...
        """
        return [41, list(self.subList.keys())]
...
while True:
    try:
        ...
        print("Recebi: %s" % pickle.loads(msg))
